...
/// elements after removing the ducplicates, then the first k elements of nums shouldd hold the
...
mod tests {
    ...
    fn test_remove_element() {
        ...
        assert_eq!(Solution::remove_element(&mut p, 3), 2);
        //assert_eq!(p, vec![2, 2]);

        let mut p = vec![0, 1, 2, 2, 3, 0, 4, 2];
        assert_eq!(Solution::remove_element(&mut p, 2), 5);
        //assert_eq!(p, vec![0, 1, 4, 0, 3]);
    }
}
